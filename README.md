...
| what an operator can see | **23 checks**: the Prometheus endpoint parsed the way a scraper parses it, its numbers moved by writing, searching and being refused, and the log turned up without a rebuild ([docs/observability.md](docs/observability.md)) | `tools/metrics_check.py` |
...
| [docs/observability.md](docs/observability.md) | the metrics a scraper reads, and the log an operator turns up |
